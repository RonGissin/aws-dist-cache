...
SEC_GRP_NODE="cl-sg-nodes-`date +'%N'`"

echo "setup firewall $SEC_GRP_NODE"
aws ec2 create-security-group   \
    --group-name $SEC_GRP_NODE       \
    --description "Allow access from manager" 

echo "setup rule allowing SSH access to $MY_IP only"
aws ec2 authorize-security-group-ingress        \
    --group-name $SEC_GRP_NODE --port 22 --protocol tcp \
    --cidr $MY_IP/32

echo "setup rule allowing HTTP (port 5000) access to $PUBLIC_IP_MANAGER only"
aws ec2 authorize-security-group-ingress        \
    --group-name $SEC_GRP_NODE --port 5000 --protocol tcp \
